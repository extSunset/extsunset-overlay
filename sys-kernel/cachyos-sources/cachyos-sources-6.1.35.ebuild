...
SRC_URI="
	...
	latency-nice? ( ${_patchsource}/misc/0001-Add-latency-priority-for-CFS-class.patch ->
	...
	hardened-bore? (
		${_patchsource}/misc/0001-hardened.patch -> 0001-hardened-${KV_FULL}.patch
	)
	hardened? (
	...
	clang-pgo? ( ${_patchsource}/misc/0001-Clang-PGO.patch -> 0001-Clang-PGO-${KV_FULL}.patch )
	movable-bar? ( ${_patchsource}/misc/0001-PCI-Allow-BAR-movement-during-boot-and-hotplug.patch ->
					 0001-PCI-Allow-BAR-movement-during-boot-and-hotplug-${KV_FULL}.patch )
	aufs? ( ${_patchsource}/misc/0001-aufs-6.1-merge-v20230109.patch ->
			  0001-aufs-6.1-merge-v20230109-${KV_FULL}.patch )
	rt? ( ${_patchsource}/misc/0001-rt.patch -> 0001-rt-${KV_FULL}.patch )
	zblock? ( ${_patchsource}/misc/0001-mm-add-zblock-new-allocator-for-use-via-zpool-API.patch ->
				0001-mm-add-zblock-new-allocator-for-use-via-zpool-API-${KV_FULL}.patch )
	spadfs? ( ${_patchsource}/misc/0001-spadfs-6.1-merge-v1.0.17.patch ->
				0001-spadfs-6.1-merge-v1.0.17-${KV_FULL}.patch )
	sched-task-classes? ( ${_patchsource}/misc/0002-sched-Introduce-IPC-classes-for-load-balance.patch ->
							0002-sched-Introduce-IPC-classes-for-load-balance-${KV_FULL}.patch )
	sched-avoid-unnecessary-migrations? ( ${_patchsource}/misc/0001-x86-sched-Avoid-unnecessary-migrations-within-SMT-do.patch ->
											0001-x86-sched-Avoid-unnecessary-migrations-within-SMT-do-${KV_FULL}.patch )
	high-hz? ( ${_patchsource}/misc/0001-high-hz.patch -> 0001-high-hz-${KV_FULL}.patch )
	thp-shrinker? ( ${_patchsource}/misc/0001-mm-introduce-THP-Shrinker.patch -> 0001-mm-introduce-THP-Shrinker-${KV_FULL}.patch )
	resizable-bar? ( ${_patchsource}/misc/enable-resizable-bar-support-nv-driver.patch ->
					   enable-resizable-bar-support-nv-driver-${KV_FULL}.patch )
"
...
REQUIRED_USE="
	^^ ( pds bmq bore cfs tt )
	tuned-bore? ( bore )
	latency-nice? ( || ( bore cfs ) )
	gcc-lto? ( !debug )
	hardened-bore? ( bore )
	high-hz? ( tt )
	sched-task-classes? ( ^^ ( cfs bore ) )
	sched-avoid-unnecessary-migrations? ( ^^ ( cfs bore ) )
"
...
src_prepare() {
	...
	if use latency-nice; then
	   if use cfs || use bore; then
		  eapply "${DISTDIR}/0001-Add-latency-priority-for-CFS-class-${KV_FULL}.patch"
	   fi
	fi
	...
	if use bore; then
		...
		if use tuned-bore; then
			eapply "${DISTDIR}/0001-bore-tuning-sysctl-${KV_FULL}.patch"
		fi

		if use hardened-bore; then
			eapply "${DISTDIR}/0001-hardened-${KV_FULL}.patch"
		fi
	fi
	...
	if use sched-avoid-unnecessary-migrations; then
		eapply "${DISTDIR}/0001-x86-sched-Avoid-unnecessary-migrations-within-SMT-do-${KV_FULL}.patch"
	fi

	if use sched-task-classes; then
		eapply "${DISTDIR}/0002-sched-Introduce-IPC-classes-for-load-balance-${KV_FULL}.patch"
	fi
	...
	if use aufs; then
		eapply "${DISTDIR}/0001-aufs-6.1-merge-v20230109-${KV_FULL}.patch"
	fi

	if use spadfs; then
		eapply "${DISTDIR}/0001-spadfs-6.1-merge-v1.0.17-${KV_FULL}.patch"
	fi
	...
	if use clang-pgo; then
		eapply "${DISTDIR}/0001-Clang-PGO-${KV_FULL}.patch"
	fi
	...
	if use movable-bar; then
		eapply "${DISTDIR}/0001-PCI-Allow-BAR-movement-during-boot-and-hotplug-${KV_FULL}.patch"
	fi

	if use resizable-bar; then
		eapply "${DISTDIR}/enable-resizable-bar-support-nv-driver-${KV_FULL}.patch"
	fi

	if use thp-shrinker; then
		eapply "${DISTDIR}/0001-mm-introduce-THP-Shrinker-${KV_FULL}.patch"
	fi

	if use zblock; then
		eapply "${DISTDIR}/0001-mm-add-zblock-new-allocator-for-use-via-zpool-API-${KV_FULL}.patch"
	fi
...
src_configure() {
	...
	active_sched = usev hardened || usev cfs || usev bore || usev tt || usev pds || usev bmq
...
pkg_postinst() {
	...
	if use clang-pgo; then
		elog "Workflow for building a kernel using Clang PGO:"
		elog "1. Compile the kernel with CONFIG_CLANG_PGO enabled (without LTO!!)"
		elog "2. Install the kernel"
		elog "3. Boot into the kernel"
		elog "4. `echo 1 | sudo tee /proc/pgo/reset`"
		elog "5. Run a workload"
		elog "6. `sudo cp -a /proc/pgo/vmlinux.profraw .`"
		elog "7.` sudo chown $USER:$USER vmlinux.profraw`"
		elog "8. `llvm-profdata merge --output=vmlinux.profdata vmlinux.profraw`"
		elog "9. export KCFLAGS=-fprofile-use=$PATHTOPROFDATA"
		elog "10. Compile the Kernel, LTO can be enabled now, include the PGO patch but disable the PGO entry"
		elog "11. Install the final kernel :)"
		elog " "
	fi
